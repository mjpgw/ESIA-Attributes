...
courses_df = load_df(courses_ws, ["Course", "Attribute(s)"])
change_log_df = load_df(change_log_ws, [
    "Course", "Old Title", "New Title", "Old Attributes", "New Attributes",
    "Comment", "Submitted By", "Timestamp", "Sent to ASO"
])
inquiry_log_df = load_df(inquiry_log_ws, [
    "Name", "Comment", "Addressed?", "Timestamp"
])

# Ensure boolean columns are valid
    ...
    if is_admin:
        ...
        with st.form("edit_form"):
            ...
            if submitted:
                ...
                courses_ws.update([courses_df.columns.tolist()] + courses_df.values.tolist())
                change_log_ws.update([change_log_df.columns.tolist()] + change_log_df.astype(str).values.tolist())
...
with tab2:
    ...
    if not change_log_df.empty:
        edited_df = st.data_editor(
            ...
            disabled=([] if is_admin else list(change_log_df.columns)),
            key="change_log_editor"
        )
        if is_admin and st.button("Save Change Log"):
            change_log_df = edited_df.copy()
            change_log_ws.update([change_log_df.columns.tolist()] + change_log_df.astype(str).values.tolist())
...
with tab3:
    ...
    filtered_df = inquiry_log_df[inquiry_log_df["Addressed?"] == False] if filter_unaddressed else inquiry_log_df

    if not filtered_df.empty:
        edited_df = st.data_editor(
            filtered_df,
            use_container_width=True,
            disabled=([] if is_admin else list(filtered_df.columns)),
            key="inquiry_log_editor"
        )
